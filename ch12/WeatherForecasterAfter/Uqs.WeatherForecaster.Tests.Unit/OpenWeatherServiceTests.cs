...
using Uqs.WeatherForecaster.Tests.Unit.Fakes;
...
public class OpenWeatherServiceTests
{
    ...
    public async Task OneCallAsync_RequiredParametersOnlyLatLonAppId_UrlIsFormattedAsExpected()
    {
        ...
        var sut = new OpenWeatherService(FAKE_KEY, fakeHttpClient);

        // Act
        var oneCallResponse = await sut.OneCallAsync(GREENWICH_LATITUDE, GREENWICH_LONGITUDE, new Excludes[0], Units.Standard);
    ...
    public async Task OneCallAsync_WithRequiredAndOptional_UrlIsFormattedAsExpected(Excludes[] excludes, Units unit, string? excludesQuery, string unitQuery)
    {
        ...
        // Act
        var oneCallResponse = await sut.OneCallAsync(GREENWICH_LATITUDE, GREENWICH_LONGITUDE, excludes ?? new Excludes[0], unit);
    ...
    public async Task OneCallAsync_UsingDailySampleFile_SerializedProperly()
    {
        ...
        // Act
        var oneCallResponse = await sut.OneCallAsync(GREENWICH_LATITUDE, GREENWICH_LONGITUDE, new Excludes[] { Excludes.Current, Excludes.Minutely, Excludes.Hourly, Excludes.Alerts }, Units.Imperial);
    ...
    public async Task OneCallAsync_NonSerializableTextContent_JsonReaderException()
    {
        ...
        // Act
        Exception ex = await Record.ExceptionAsync(() => _ = sut.OneCallAsync(GREENWICH_LATITUDE, GREENWICH_LONGITUDE, new Excludes[0], Units.Imperial));
