...
{
    private IOpenWeatherService _openWeatherServiceMock = Mock.Of<IOpenWeatherService>();
    ...
    public async Task GetForecastWeatherAnalysis_LatAndLonPassed_ReceivedByOpenWeatherAccurately()
    {
        ...
        Mock.Get(_openWeatherServiceMock)
           .Setup(x => x.OneCallAsync(It.IsAny<decimal>(), It.IsAny<decimal>(), It.IsAny<IEnumerable<Excludes>>(), It.IsAny<Units>()))
           .Callback<decimal, decimal, IEnumerable<Excludes>, Units>((lat, lon, _, _) => { actualLat = lat; actualLon = lon; })
    ...
    public async Task GetForecastWeatherAnalysis_RequestsToOpenWeather_MetricUnitIsUsed()
    {
        ...
        Mock.Get(_openWeatherServiceMock)
           .Setup(x => x.OneCallAsync(It.IsAny<decimal>(), It.IsAny<decimal>(), It.IsAny<IEnumerable<Excludes>>(), It.IsAny<Units>()))
           .Callback<decimal, decimal, IEnumerable<Excludes>, Units>((_, _, _, x) => actualUnit = x)
           .Returns(Task.FromResult(GetSample(_defaultTemps)));
